...
int main(void) {
    ...
    // says key
    printf("ioctl(fd, UI_SET_EVBIT, EV_KEY)\n");
    if(ioctl(fd, UI_SET_EVBIT, EV_KEY) < 0)
        die("error: ioctl EV_KEY");

    // says mouse
    printf("ioctl(fd, UI_SET_KEYBIT, BTN_LEFT)\n");
    if(ioctl(fd, UI_SET_KEYBIT, BTN_LEFT) < 0)
        die("error: ioctl BTN_LEFT");

    printf("ioctl(fd, UI_SET_EVBIT, EV_REL)\n");
    if(ioctl(fd, UI_SET_EVBIT, EV_REL) < 0)
        die("error: ioctl EV_REL");

    printf("ioctl(fd, UI_SET_RELBIT, REL_X)\n");
    if(ioctl(fd, UI_SET_RELBIT, REL_X) < 0)
        die("error: ioctl REL_X");

    printf("ioctl(fd, UI_SET_RELBIT, REL_Y)\n");
    ...
    while(1) {
        ...
        // terminate if user hits ESC or Ctrl-D
        if (kbhit()) {
          i = getchar();
